	...
	incSpecs := []incSpec{
		{
			...
			symptoms:      "Pod enters CrashLoopBackOff with increasing backoff delays.\nContainer last state shows OOMKilled (exit code 137).",
			rootCause:     "Memory limit set too low for peak traffic.\nJava heap + off-heap exceeds container memory limit during garbage collection spikes.",
			sol:           "1. Increase memory limit from 256Mi to 512Mi\n2. Set -XX:MaxRAMPercentage=75 to prevent JVM from consuming entire cgroup limit\n3. Add VPA recommendations",
		...
		},
		{
			...
			symptoms:      "Pod crashes immediately after starting.\nLogs show 'failed to load configuration' or 'missing required environment variable'.",
			...
			sol:           "1. Verify all referenced ConfigMaps and Secrets exist\n2. Check `kubectl describe pod` for missing volume mounts\n3. Rollout restart after fixing config:\n   `kubectl rollout restart deployment/<name> -n <ns>`",
		...
		},
		{
			...
			symptoms:      "All HTTPS endpoints return 503.\nBrowser shows NET::ERR_CERT_DATE_INVALID.\ncert-manager Certificate resource shows Ready=False.",
			rootCause:     "cert-manager ClusterIssuer ACME account key was rotated without updating the secret.\nRenewal failed silently 30 days before expiry.",
			sol:           "1. Delete the TLS secret to trigger immediate re-issuance\n2. Verify ClusterIssuer has valid ACME account\n3. Set up certificate expiry monitoring at 14-day threshold",
		...
		},
		{
			...
			symptoms:      "CNPG PostgreSQL pod reports PVC usage above 90%.\nWrite operations start failing with 'could not extend file' errors.",
			rootCause:     "WAL retention + bloated tables from missing VACUUM.\nautovacuum was disabled during a migration and not re-enabled.",
			sol:           "1. Run VACUUM FULL on bloated tables\n2. Ensure autovacuum is enabled\n3. Expand PVC:\n   `kubectl edit pvc data-customer-db-0`\n   (if storage class supports expansion)\n4. Set up alerts at 80% threshold",
		...
		},
		{
			...
			symptoms:      "Node shows NotReady status.\nPods on the node are evicted.\nkubelet is not responding to API server.",
			rootCause:     "Kernel OOM killer terminated kubelet due to memory pressure from a daemonset with no resource limits.",
			sol:           "1. SSH to node, restart kubelet:\n   `systemctl restart kubelet`\n2. Set resource limits on all daemonsets\n3. Consider using system-reserved and kube-reserved kubelet flags",
		...
		},
		{
			...
			symptoms:      "New pods stuck in ImagePullBackOff.\nEvents show 'unauthorized: authentication required' for ghcr.io images.",
			rootCause:     "Image pull secret expired.\nGHCR personal access token used in imagePullSecret had a 90-day expiry.",
			sol:           "1. Rotate the image pull secret with a new PAT\n2. Use workload identity or managed identity instead of static tokens\n3. Set up token expiry monitoring",
		...
		},
		{
			...
			symptoms:      "Multiple services report increased p99 latency.\nDNS lookups intermittently fail with SERVFAIL.\nCoreDNS pods show high CPU usage.",
			rootCause:     "CoreDNS ndots:5 default causing excessive DNS queries.\nEach service-to-service call generates 5+ DNS lookups before resolving.",
			sol:           "1. Set `dnsConfig.options: [{name: ndots, value: '2'}]` in pod specs for services that call external domains\n2. Scale CoreDNS with HPA\n3. Enable DNS caching in application",
		...
		},
		{
			...
			symptoms:      "kubectl commands hang or timeout.\nAPI server returns 'etcdserver: leader changed' errors.\nNew pods cannot be scheduled.",
			...
			sol:           "1. Check etcd member health:\n   `etcdctl endpoint health`\n2. Verify network connectivity between members\n3. If quorum lost, restore from backup\n4. Add etcd health to maintenance checklist",
		...
		},
		{
			...
			services: []string{"order-api"}, clusters: []string{"prod-eu-1"}, namespaces: []string{"commerce"},
			symptoms:      "HPA rapidly scales between min and max replicas every 2-3 minutes.\nCPU oscillates around the target threshold.",
			rootCause:     "HPA target CPU utilization set to 50% with default stabilization window.\nBursty traffic pattern triggers constant rescaling.",
			sol:           "1. Increase `behavior.scaleDown.stabilizationWindowSeconds` to 300\n2. Set scaleDown rate limit to 1 pod per 60s\n3. Consider using KEDA with a longer polling interval",
		...
		},
		{
			...
			services: []string{"customer-db"}, clusters: []string{"prod-eu-1"}, namespaces: []string{"database"},
			symptoms:      "StatefulSet pods stuck in Pending.\nPVC shows status Pending.\nEvents show 'waiting for first consumer to be created' or 'no persistent volumes available'.",
			rootCause:     "Storage class provisioner ran out of capacity in the availability zone.\nLonghorn storage pool exhausted.",
			sol:           "1. Check storage pool:\n   `kubectl get nodes.longhorn.io -n longhorn-system`\n2. Add disk capacity or a new node\n3. For immediate fix: create PV manually\n4. Set up storage capacity alerting",
